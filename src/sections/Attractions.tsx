...
const AttractionCard = ({ item, index }: { item: any; index: number }) => {
  ...
  return (
    <div
      ref={cardRef}
      className="relative w-full h-[60vh] lg:h-[80vh] overflow-hidden mb-8 last:mb-0"
    >
      <img
        ref={bgRef}
        src={item.image}
      ...
      />
      <div className="absolute inset-0 z-10 bg-linear-to-t from-moa-black/90 via-moa-black/20 to-transparent" />

      <div className="absolute inset-0 z-20 flex flex-col justify-end p-8 lg:p-16">
        <div className="max-w-3xl">
          <span className="text-moa-gold text-xs font-bold uppercase tracking-[0.2em] mb-4 block">
            0{index + 1}
          </span>
          <h3 className="font-display text-4xl lg:text-7xl text-white mb-6 leading-none">
            {item.name}
          </h3>
...
export function Attractions() {
  return (
    <section
      id="entertainment"
      className="relative w-full bg-moa-black pt-32 pb-48"
    >
      <div className="max-w-7xl mx-auto px-6 mb-24 text-center">
        <h2 className="font-display text-4xl lg:text-7xl text-white mb-8">
          The Only Mall on Earth
          <br />
          with a Roller Coaster Inside.
        </h2>
        <p className="text-xl text-white/60 font-light max-w-2xl mx-auto">
          Retail draws a crowd. Entertainment creates a destination. Our
          immersive footprint guarantees an unprecedented multi-hour dwell time.
        </p>
      </div>
