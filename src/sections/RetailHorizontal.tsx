...
export function RetailHorizontal() {
  ...
  return (
    <section
      id="retail"
      ref={sectionRef}
      className="relative w-full min-h-screen lg:h-screen bg-moa-black overflow-hidden flex items-center"
    >
      <div className="absolute top-12 left-6 lg:left-12 z-30 pointer-events-none">
        <h2 className="font-display font-light text-xl lg:text-2xl text-white mix-blend-difference hidden md:block">
      ...
      <div
        ref={scrollRef}
        className="flex flex-col lg:flex-row h-full lg:w-[400vw]"
      >
          ...
          >
            ...
            <div className="relative z-10 w-full max-w-5xl grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12 items-center">
              <div className="md:col-span-10 lg:col-span-7 backdrop-blur-md bg-black/40 p-8 lg:p-12 border border-white/5 rounded-2xl shadow-2xl">
                <div className="text-moa-gold text-[10px] md:text-xs font-bold uppercase tracking-[0.3em] mb-4 md:mb-6">
                  {panel.category}
                </div>
                ...
                <div className="space-y-4 md:space-y-6 border-l border-moa-gold/40 pl-6">
                  <span className="block text-[9px] md:text-[10px] uppercase font-bold text-moa-gold tracking-[0.2em]">
                    Notable Partners
                  </span>
                  <div className="flex flex-wrap gap-4 md:gap-8 items-center">
                    {panel.anchors.map((anchor) => {
                      const name =
                        typeof anchor === "string" ? anchor : anchor.name;
                      const localLogo =
                        typeof anchor === "string" ? null : anchor.logo;
                      const domain =
                        DOMAINS[name] ||
                      ...
                      return (
                        <div
                          key={name}
                          className="group/brand flex flex-col items-center gap-2"
                        >
                            ...
                            <img
                              ...
                              onError={(e) => {
                                ...
                                (
                                  ...
                                  `<span class="text-[8px] font-bold text-white/40 text-center">${name}</span>`;
                              }}
                            />
                          </div>
                        </div>
      ...
      <div className="hidden lg:block absolute bottom-12 left-12 right-12 h-[1px] bg-white/10 z-20 overflow-hidden rounded-full">
        <div
          className="h-full bg-moa-gold origin-left scale-x-0 rounded-full"
          id="retail-progress"
        />
      </div>
    </section>
